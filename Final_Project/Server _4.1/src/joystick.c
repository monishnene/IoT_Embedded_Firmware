...
{
	static uint8_t previous_signal = 0;
	uint32_t ADC_data=0;
	ADC0->SINGLEFIFOCLEAR |= ADC_SINGLEFIFOCLEAR_SINGLEFIFOCLEAR;
	ADC_data = ADC_DataSingleGet(ADC0);
	if((ADC_data < AHEAD_MAX)&&(ADC_data > AHEAD_MIN))
	{
		if(previous_signal != front)
		{
			black_box_update(front);
		}
		previous_signal = front;
	}
	else if((ADC_data < BACK_MAX)&&(ADC_data > BACK_MIN))
	{
		if(previous_signal != behind)
		{
			black_box_update(behind);
		}
		previous_signal = behind;
	}
